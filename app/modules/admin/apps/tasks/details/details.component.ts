...
export class TasksDetailsComponent implements OnInit, AfterViewInit, OnDestroy
{
    ...
    ngOnInit(): void
    {
        ...
        if(!(this._activatedRoute.snapshot.params.id == 'task' || this._activatedRoute.snapshot.params.id == 'section')) {
            this.getTasks();
        }else {
            this.task = {
                id:'01',
                type: this._activatedRoute.snapshot.params.id,
                title:'',
                notes:'',
                tags:[]
            }
    ...
    isOverdue(): boolean
    {
        return moment(this.task.endDate, moment.ISO_8601).isBefore(moment(), 'days');
    }
