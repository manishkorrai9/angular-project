...
@Component({
  ...
  templateUrl: './sleep-logs.component.html',
  styleUrls: ['./sleep-logs.component.scss']
})
export class SleepLogsComponent implements OnInit {
  ...
  console.log(labels);
    ...
    setWeekdata(data: any, name: string): any {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        if (moment(data[i].start_date).format('DD').toLocaleLowerCase() == name.toLocaleLowerCase()) {
          obj = data[i];
          break;
        }
      }
    ...
    getSteps() {
      let type = 2;
      const url = `api/PatientRegistration/GetPatient_SleepPattern_last30days?patientId=${this.data.userId}`;
      this.httpService
        .getAll(url)
        .subscribe((res: any) => {
          if (res.data) {
            ...
            if (type == 1) {
              this.weekNames.forEach((weekDataObj) => {
                ...
                if (weekobj.start_date) {
                  data.push({
                    x: moment(weekobj.start_date).format('DD'),
                    y: weekobj.hours ? weekobj.hours : null,
                  });
                } else {
                  data.push({
                    x: weekDataObj,
              ...
              res.data.forEach((dataObj) => {
                ...
                if (dataObj.hours) {
                  data.push({
                    x: moment(dataObj.start_date).format('DD'),
                    y: dataObj.hours ? dataObj.hours : null,
                  });
                }
              });
            }
