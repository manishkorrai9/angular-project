...
@Component({
  selector: 'app-steps-logs',
  templateUrl: './steps-logs.component.html',
  styleUrls: ['./steps-logs.component.scss']
})
export class StepsLogsComponent implements OnInit {
...
console.log(labels);
  ...
  setWeekdata(data: any, name: string): any {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (moment(data[i].steps_start_on).format('DD').toLocaleLowerCase() == name.toLocaleLowerCase()) {
        obj = data[i];
        break;
      }
    }
  ...
  getSteps() {
    let type = 2;
    const url = `api/PatientRegistration/GetPatientStepsTrackingDetails_Last30Days?patientId=${this.data.userId}`;
    this.httpService
      .getAll(url)
      .subscribe((res: any) => {
        if (res.data) {
          ...
          if (type == 1) {
            this.weekNames.forEach((weekDataObj) => {
              ...
              if (weekobj.steps_start_on) {
                data.push({
                  x: moment(weekobj.steps_start_on).format('DD'),
                  y: weekobj.steps_count ? weekobj.steps_count : null,
                });
              } else {
                data.push({
                  x: weekDataObj,
            ...
            res.data.forEach((dataObj) => {
              ...
              if (dataObj.steps_count) {
                data.push({
                  x: moment(dataObj.steps_start_on).format('DD'),
                  y: dataObj.steps_count ? dataObj.steps_count : null,
                });
              }
            });
          }
